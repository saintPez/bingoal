...
const createCarton = async () => {
    ...
    for (let i = 0; i < 5; i++) {
        result.push(...B.splice(Math.floor(Math.random() * (B.length - 0)) + 0, 1));
    }
    for (let i = 0; i < 5; i++) {
        result.push(...I.splice(Math.floor(Math.random() * (I.length - 0)) + 0, 1));
    }
    for (let i = 0; i < 5; i++) {
        if (i === 2) result.push(0);
        else result.push(...N.splice(Math.floor(Math.random() * (N.length - 0)) + 0, 1));
    }
    for (let i = 0; i < 5; i++) {
        result.push(...G.splice(Math.floor(Math.random() * (G.length - 0)) + 0, 1));
    }
    for (let i = 0; i < 5; i++) {
